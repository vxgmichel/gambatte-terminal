...
SCALE_MAX = int(os.environ.get("GAMBATERM_SCALE_MAX", "10"))

# Band layout table: (V, rows_per_band) per scale for kitty.
# Generated by tools/bandsize-estimate.py using the calibration frame
# (PCG64, seed=42).  V=1 means the full frame fits in one band.
BAND_LAYOUT = {
    1: (1, 144),
    2: (2, 142),
    3: (2, 86),
...
MAX_FRAME_TRANSMISSION_SIZE = int(
    os.environ.get("GAMBATERM_MAX_FRAME_TRANSMISSION_SIZE", str(128 * 1024))
)
...
class FrameEncoder:
    ...
    def __init__(
        ...
        self._max_frame_transmission_size = MAX_FRAME_TRANSMISSION_SIZE
        self._V = 0
        self._v_current = 0
        self._v_row_height = 144
    ...
    def encode_kitty(
        ...
        """
        if self.is_banding:
            return self._encode_kitty_band(video, last_frame)
    ...
    def _ensure_band_layout(self) -> None:
        """Populate ``_V`` and ``_v_row_height`` from the band layout table."""
        entry = BAND_LAYOUT.get(self.kitty_scale)
        if entry is None:
            self._V = 1
            self._v_row_height = 144
        else:
            self._V, self._v_row_height = entry
        self._v_current = 0

    @property
    def is_banding(self) -> bool:
        return self._frame_banding and self._V > 1

    def _encode_kitty_band(
        self,
        video: np.ndarray,
        last_frame: np.ndarray | None,
    ) -> bytes:
        """Encode one band of a kitty frame cycle.

        Each band is placed at its own cell-snapped cursor position with
        sub-cell X/Y offsets for pixel-precise alignment, matching the
        existing dirty-rect delta positioning strategy.
        """
        t0 = time.perf_counter()
        self.kitty_frame_no += 1
        n = self.kitty_frame_no

        y0 = self._v_current * self._v_row_height
        y1 = min(y0 + self._v_row_height, 144)

        band_video = video[y0:y1, :]
        # When _last_rendered is the zero sentinel, never skip.
        if last_frame is not None and last_frame.shape == video.shape and np.any(last_frame):
            if np.array_equal(band_video, last_frame[y0:y1, :]):
                self._v_current = (self._v_current + 1) % self._V
                self._skipped += 1
                self._log_frame(n, "0.0", "band_skip", 0, 0,
                                f"v={self._v_current}")
                return b""

        rgba = to_rgba_u8(band_video)
        if self.kitty_scale > 1:
            rgba = np.repeat(
                np.repeat(rgba, self.kitty_scale, axis=0),
                self.kitty_scale, axis=1,
            )
        band_h = rgba.shape[0]
        band_w = rgba.shape[1]

        # Cell-snapped cursor position.
        py = y0 * self.kitty_scale
        px = 0
        row = self.refx_kitty + py // self.cell_h
        col = self.refy_kitty + px // self.cell_w
        off_y = py % self.cell_h
        ...
        image_id = self._v_current + 1

        # Kitty pixel-offset scroll bug workaround.
        if self.force_transparent_offset and (off_y or off_x):
            padded = np.zeros(
                (band_h + off_y, band_w + off_x, 4), dtype=np.uint8,
            )
            padded[off_y:off_y + band_h, off_x:off_x + band_w] = rgba
            rgba = padded
            band_w += off_x
            band_h += off_y
            off_x = 0
        ...
        # Delete old placement of this band before re-placing.
        prefix = f"\033_Ga=d,d=i,i={image_id}\033\\".encode()
        result = (
            prefix
            + f"\033[{row};{col}H".encode()
            + encode_kitty_rgba(
                rgba.ravel(), band_w, band_h,
                image_id=image_id, X=off_x, Y=off_y,
            )
        )
        self._keyframes += 1
        self._bytes_out += len(result)
        ...
        elapsed_us = int((time.perf_counter() - t0) * 1e6)
        self._log_frame(
            n, "0.0", "band", len(result), elapsed_us,
            f"{row},{col},{self.cell_h},{self.cell_w},,,"
            f",,{y0},{y1 - y0},"
            f"{self.refx_kitty},{self.refy_kitty},{self.kitty_scale}",
        )
        return result
...
class GraphicsRenderer:
    ...
    def _make_encoder(self) -> None:
        ...
        self.encoder = FrameEncoder(
            ...
            frame_banding=self._frame_banding,
        )
    ...
    @property
    def kitty_scale(self) -> int | None:
        if self.scaler is None:
            return None
        return self.scaler.kitty_scale
    ...
    def render(
        ...
        if self.scaler is None:
            ...
            self._make_encoder()
            if self._frame_banding and self.encoder is not None:
                self.encoder._ensure_band_layout()

        # Use the last rendered frame as the comparison reference.
        # Sentinel zeros on first call so every band renders initially.
        if self._last_rendered is None:
            self._last_rendered = np.zeros_like(video)
        last_frame = self._last_rendered
        ...
        if protocol is GraphicsProtocol.SIXEL:
            ...
            v_before = self.encoder._v_current
            ...
            if self.encoder._v_current != v_before and result:
                y0 = v_before * self.encoder._v_row_height
                y1 = min(y0 + self.encoder._v_row_height, video.shape[0])
                self._last_rendered[y0:y1, :] = video[y0:y1, :]
